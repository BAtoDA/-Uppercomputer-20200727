...
namespace 欧姆龙Fins协议.欧姆龙.报文处理
{
    ...
    public class OmronFinsUDP : PLC_public_Class, IPLC_interface
    {
        ...
        public OmronFinsUDP(IPEndPoint iPEndPoint, string pattern)//构造函数---初始化---open
        {
            OmronFinsUDP.IPEndPoint = iPEndPoint;//实例化IP地址
        ...
        public  void readResultRender<T>(OperateResult<T> result, string address, ref string Data)
        {
            ...
            if (result.IsSuccess != true)//指示读取失败
            {
                ...
                if (retry == 1)
                    ShowErrorNotifier(DateTime.Now.ToString("[HH:mm:ss] ") + $"[{address}] 读取失败{Environment.NewLine}原因：{result.ToMessageShowString()}");
                if (retry >= 1)
