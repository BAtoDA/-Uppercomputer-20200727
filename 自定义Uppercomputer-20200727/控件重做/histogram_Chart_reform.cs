...
namespace 自定义Uppercomputer_20200727.控件重做
{
    ...
    class histogram_Chart_reform :UI_Library_da.histogram_Chart, ControlCopy, TextBox_base
    {
        ...
        public Control Objectproperty(string Name, Form form)
        {
            ...
            {
                //获取上个控件的值
                string path = this.Parent?.ToString() ?? doughnut_Chart_ID ;
                path += "- " + this.Name;
                var parameter = db.histogram_Chart_parameter.Where(pi => pi.ID.Trim() == path).FirstOrDefault();
                ...
                var contrclass = db.histogram_Chart_Class.Where(pi => pi.ID.Trim() == path).FirstOrDefault();
                var contrcolor = db.Button_colour.Where(pi => pi.ID.Trim() == path).FirstOrDefault();

                //产生新的控件
                histogram_Chart_reform control = (histogram_Chart_reform)this.Clone();
