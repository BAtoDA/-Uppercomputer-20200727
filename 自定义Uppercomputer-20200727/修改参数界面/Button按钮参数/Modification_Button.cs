...
namespace 自定义Uppercomputer_20200727.修改参数界面
{
    ...
    public partial class Modification_Button : Skin_VS
    {
        ...
        public Modification_Button(string ID, object all_purpose)
        {
            ...
            skinTextBox8.Text = Button_ID + "-" + ((Button_reform)this.all_purpose).Name;//保存ID 为一地址  -是分割字符 
        ...
        {
            //查询数据库是否有该数据
            if (Button_EFbase.Button_Parameter_inquire<Button_Class>(this.skinTextBox8.Text) == "OK")
            {
                Button_EFbase button_EF = new Button_EFbase();//实例化EF对象
                button = button_EF.Button_Parameter_Query<Button_Class>(this.skinTextBox8.Text);//获取按钮类全部参数
                List_Index();//开始改变索引
            ...
            {
                //新增控件获取位置与大小
                this.skinTextBox1.Text = ((Button_reform)this.all_purpose).Location.X.ToString();//控件位置X轴
                this.skinTextBox2.Text = ((Button_reform)this.all_purpose).Location.Y.ToString();//控件位置Y轴
                this.skinTextBox4.Text = ((Button_reform)this.all_purpose).Size.Width.ToString();//控件大小宽度
                this.skinTextBox3.Text = ((Button_reform)this.all_purpose).Size.Height.ToString();//控件大小宽度
                if (skinRadioButton1.Checked == false & skinRadioButton4.Checked == false) skinRadioButton1.Checked = true;
        ...
        #endregion
        private void Modification_Button_Load(object sender, EventArgs e)//加载窗口
        {
           ...
           (new List<SkinTabPage> { this.skinTabPage1, this.skinTabPage2, this.skinTabPage3, this.skinTabPage4, this.skinTabPage5 }, ((Button_reform)this.all_purpose).Name, ((Button_reform)this.all_purpose).Parent.ToString() + "-" + ((Button_reform)this.all_purpose).Name);//调用加载数据类                
        }

        private void Modification_Button_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void List_Index()
        {
            ...
            ((SkinButton)this.all_purpose).Text = button.Control_state_0_content.Trim();
        ...
        private void skinButton2_Click(object sender, EventArgs e)//用户点击了保存
        {
            //先判断用户输入的数据
            ...
            ((Button_reform)this.all_purpose).Text = this.skinChatRichTextBox1.Text.Trim();//设置文本
            Public_attributeCalss public_AttributeCalss = new Public_attributeCalss();//实例化按钮参数设置
            public_AttributeCalss.attributeCalss((Button_reform)this.all_purpose, button_EF.Button_Parameter_Query<Button_Class>(skinTextBox8.Text.Trim()));//查询数据库--进行设置后的参数修改
            Add_to_allow = true;//允许添加控件
            this.Close();
            this.Dispose();
        }
        private Button_parameter button_Parameter()//获取要写入的一般参数
        {
            return new Button_parameter
            {
                ...
                读写设备 = this.skinComboBox13.Text.Trim(),
        ...
        private Tag_common_parameters tag_Common_Parameters()//获取要写入的标签参数
        {
            ...
            if (Convert.ToInt32(this.skinComboBox2.Text) == 1)//判断当前用户选中的格式状态
            {
                ...
                tag_Common = new Tag_common_parameters
                {
                    ...
                    Control_type = ((Button_reform)this.all_purpose).Name,
                    FROM = From_Name,
