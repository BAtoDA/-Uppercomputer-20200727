...
namespace 自定义Uppercomputer_20200727.修改参数界面.GroupBox四边形方框
{
    public partial class Modification_GroupBox : Skin_VS
    {
        private string Button_ID;//控件的基本信息
        private object all_purpose;//通用类型
        public bool Add_to_allow = false;//反馈标志位是否允许添加
        GroupBox_Class label;//按钮类全部参数
        private string From_Name;//控件处在的窗口名称
        public Modification_GroupBox(string ID, object all_purpose)
        {
            InitializeComponent();
            this.Button_ID = ID;//获取参数
            this.all_purpose = all_purpose;
            this.skinTextBox1.Text = Button_ID + "-" + ((GroupBox_reform)all_purpose).Name;//保存ID 为一地址  -是分割字符
        ...
        #endregion
        private void Modification_GroupBox_Load(object sender, EventArgs e)
        {
            AnimateWindow(this.Handle, 1000, AW_SLIDE | AW_ACTIVE | AW_VER_NEGATIVE);
            Modification_label_Class _Label_Class = new Modification_label_Class(new List<SkinTabPage>() { this.skinTabPage1, this.skinTabPage2, this.skinTabPage3 }, ((GroupBox_reform)all_purpose).Name);//加载

        }
        private void Modification_GroupBox_FormClosing(object sender, FormClosingEventArgs e)
        {
            AnimateWindow(this.Handle, 1000, AW_SLIDE | AW_ACTIVE | AW_VER_NEGATIVE);
        }

        private void Modification_GroupBox_Shown(object sender, EventArgs e)
        {
            //设置默认背景颜色
            ...
            {
                //新增控件获取位置与大小
                this.textBox1.Text = ((GroupBox_reform)this.all_purpose).Location.X.ToString();//控件位置X轴
                this.skinTextBox2.Text = ((GroupBox_reform)this.all_purpose).Location.Y.ToString();//控件位置Y轴
                this.skinTextBox4.Text = ((GroupBox_reform)this.all_purpose).Size.Width.ToString();//控件大小宽度
                this.skinTextBox3.Text = ((GroupBox_reform)this.all_purpose).Size.Height.ToString();//控件大小宽度
            }
        ...
        private GroupBox_parameter Label_Parameter()//获取要写入的主参数
        {
            return new GroupBox_parameter
            {
        ...
        private Tag_common_parameters tag_Common_Parameters()//获取要写入的标签参数
        {
            return new Tag_common_parameters
            {
                ...
                Control_type = ((GroupBox_reform)this.all_purpose).Name,
                FROM = From_Name,
                ID = skinTextBox1.Text
            };
        }
