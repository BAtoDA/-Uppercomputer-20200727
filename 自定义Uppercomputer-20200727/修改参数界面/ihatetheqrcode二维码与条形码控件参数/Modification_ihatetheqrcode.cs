...
{
    public partial class Modification_ihatetheqrcode : Skin_VS
    {
        private string Button_ID;//控件的基本信息
        private object all_purpose;//通用类型
        public bool Add_to_allow = false;//反馈标志位是否允许添加
        ihatetheqrcode_Class button;//按钮类全部参数
        private string From_Name;//控件处在的窗口名称
        public Modification_ihatetheqrcode(string ID, object all_purpose)
        {
            InitializeComponent();
            this.Button_ID = ID;//获取参数
            this.all_purpose = all_purpose;
            this.skinTextBox8.Text = Button_ID.Trim() + "- " + ((ihatetheqrcode_reform)all_purpose).Name;//保存ID 为一地址  -是分割字符 
        ...
        #endregion
        private void Modification_ihatetheqrcode_Load(object sender, EventArgs e)
        {
            ...
            {this.skinTabPage1, this.skinTabPage2, this.skinTabPage3, this.skinTabPage4,this.skinTabPage5,this.skinTabPage6}, ((ihatetheqrcode_reform)all_purpose).Name);
        }
        ...
        private void Modification_ihatetheqrcode_Shown(object sender, EventArgs e)
        {
            ...
            {
                //新增控件获取位置与大小
                this.textBox2.Text = ((ihatetheqrcode_reform)this.all_purpose).Location.X.ToString();//控件位置X轴
                this.textBox1.Text = ((ihatetheqrcode_reform)this.all_purpose).Location.Y.ToString();//控件位置Y轴
                this.skinTextBox4.Text = ((ihatetheqrcode_reform)this.all_purpose).Size.Width.ToString();//控件大小宽度
                this.skinTextBox3.Text = ((ihatetheqrcode_reform)this.all_purpose).Size.Height.ToString();//控件大小宽度
                this.textBox3.Text = "183";//生成图片宽
                this.textBox4.Text = "134";//生成图片高
                this.skinCheckBox2.Checked = false;//默认二维码
            }
        }
        private void List_Index()
        {
            ...
            //图形参数
            this.textBox3.Text = point_or_Size(button.显示宽_高)[0].ToString()??"183";//生成图片宽
            this.textBox4.Text = point_or_Size(button.显示宽_高)[1].ToString();//生成图片高
            this.skinCheckBox2.Checked = button.二维码_条形码;//默认二维码
            //获取控件位置-大小-等信息
        ...
        private void Modification_ihatetheqrcode_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private ihatetheqrcode_parameter numerical_Parameter()//获取要写入的一般参数
        {
            return new ihatetheqrcode_parameter
            {
                ...
                显示宽_高 = this.textBox3.Text + "-" + this.textBox4.Text,
                二维码_条形码 = this.skinCheckBox2.Checked
            };
        }
        private Tag_common_parameters tag_Common_Parameters()//获取要写入的标签参数
        {
            return new Tag_common_parameters
            {
                ...
                Control_state_0_content = skinChatRichTextBox1.Text ?? ((ihatetheqrcode_reform)this.all_purpose).Name,
                //写入1状态
                Control_state_1 = 1,
                ...
                Control_type = ((ihatetheqrcode_reform)this.all_purpose).Name,
                FROM = From_Name,
