...
{
    public partial class Modification_Conveyor : Skin_VS
    {
        private string Button_ID;//控件的基本信息
        private object all_purpose;//通用类型
        public bool Add_to_allow = false;//反馈标志位是否允许添加
        Conveyor_Class button;//按钮类全部参数
        private string From_Name;//控件处在的窗口名称
        public Modification_Conveyor(string ID, object all_purpose)
        {
            InitializeComponent();
            this.Button_ID = ID;//获取参数
            this.all_purpose = all_purpose;
            this.skinTextBox8.Text = Button_ID.Trim() + "- " + ((Conveyor_reform)all_purpose).Name;//保存ID 为一地址  -是分割字符 
        ...
        private void Modification_Conveyor_Shown(object sender, EventArgs e)
        {
            //查询数据库是否有该数据
            if (Conveyor_EF.Conveyor_Parameter_inquire(this.skinTextBox8.Text) == "OK")
            {
                Conveyor_EF Conveyor_EF = new Conveyor_EF();//实例化EF对象
                button = Conveyor_EF.Conveyor_Parameter_Query(this.skinTextBox8.Text);//获取按钮类全部参数
                List_Index();//开始改变索引
            }
            else
            {
                //新增控件获取位置与大小
                this.textBox2.Text = ((Conveyor_reform)this.all_purpose).Location.X.ToString();//控件位置X轴
                this.textBox1.Text = ((Conveyor_reform)this.all_purpose).Location.Y.ToString();//控件位置Y轴
                this.skinTextBox4.Text = ((Conveyor_reform)this.all_purpose).Size.Width.ToString();//控件大小宽度
                this.skinTextBox3.Text = ((Conveyor_reform)this.all_purpose).Size.Height.ToString();//控件大小宽度
            }
        }
        private void List_Index()
        {
            ...
            //新增参数
            skinCheckBox3.Checked = button.设备控制方向 > 0 ? true : false;
            if (skinCheckBox3.Checked) skinGroupBox7.Visible = true;
            skinCheckBox2.Checked = button.设备读取数据控制 > 0 ? true : false;
            if (skinCheckBox2.Checked) skinGroupBox6.Visible = true;
            indexes.Button_ComboBoxIndex_fill(button.运输带颜色, ref this.colorComboBox3);
            indexes.Button_ComboBoxIndex_fill(button.运输带方向, ref this.skinComboBox2);
            skinTextBox5.Text = button.运输带高度.ToString();
            skinTextBox9.Text = button.运输带速度.ToString();
            skinTextBox10.Text = button.运输带角度.ToString();
            //按钮安全控制时间
        ...
        private void skinButton2_Click(object sender, EventArgs e)
        {
            ...
            if (skinTextBox5.Text.ToInt32() < 1 || skinTextBox5.Text.ToInt32() > 1000 || skinTextBox9.Text.ToInt32() < 1 || skinTextBox9.Text.ToInt32() > 1000
               || skinTextBox10.Text.ToInt32() < -90|| skinTextBox10.Text.ToInt32() > 90)
            {
            ...
            public_AttributeCalss.Conveyor((Conveyor_reform)this.all_purpose, Conveyor_EF.Conveyor_Parameter_Query(this.skinTextBox8.Text));//查询数据库--进行设置后的参数修改
            Add_to_allow = true;
            this.Close();
            this.Dispose();
        }

        private void Modification_Conveyor_FormClosing(object sender, FormClosingEventArgs e)
        {

        }

        private void skinCheckBox3_CheckedChanged(object sender, EventArgs e)//设备控制
        {
            skinGroupBox7.Visible = ((SkinCheckBox)sender).Checked;
        }

        private void skinCheckBox2_CheckedChanged(object sender, EventArgs e)//设备读取数据控制
        {
            skinGroupBox6.Visible = ((SkinCheckBox)sender).Checked;
        }
        private Conveyor_parameter numerical_Parameter()//获取要写入的一般参数
        {
            return new Conveyor_parameter
            {
                ...
                设备控制方向 = skinCheckBox3.Checked.ToInt32(),
                设备读取数据控制 = skinCheckBox2.Checked.ToInt32(),
                运输带方向 = skinComboBox2.Text,
                运输带角度 = skinTextBox10.Text.ToInt32(),
                运输带速度 = skinTextBox9.Text.ToInt32(),
                运输带高度 = skinTextBox5.Text.ToInt32(),
                运输带颜色 = colorComboBox3.Text
            };
        }
        private Tag_common_parameters tag_Common_Parameters()//获取要写入的标签参数
        {
            return new Tag_common_parameters
            {
                ...
                Control_state_0_content = skinChatRichTextBox1.Text ?? ((Conveyor_reform)this.all_purpose).Name,
                //写入1状态
                Control_state_1 = 1,
                ...
                Control_type = ((Conveyor_reform)this.all_purpose).Name,
                FROM = From_Name,
