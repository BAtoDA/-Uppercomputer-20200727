...
{
    public partial class Modification_RadioButton : Skin_VS
    {
        private string Button_ID;//控件的基本信息
        private object all_purpose;//通用类型
        public bool Add_to_allow = false;//反馈标志位是否允许添加
        RadioButton_Class button;//切换开关类全部参数
        private string From_Name;//控件处在的窗口名称
        public Modification_RadioButton(string ID, object all_purpose)
        {
            InitializeComponent();
            this.Button_ID = ID;//获取参数
            this.all_purpose = all_purpose;
            skinTextBox8.Text = Button_ID + "-" + ((RadioButton_reform)this.all_purpose).Name;//保存ID 为一地址  -是分割字符 
        ...
        private void Modification_RadioButton_Load(object sender, EventArgs e)
        {
           ...
           (new List<SkinTabPage> { this.skinTabPage1, this.skinTabPage2, this.skinTabPage3, this.skinTabPage4, this.skinTabPage5 }, ((RadioButton_reform)this.all_purpose).Name, ((RadioButton_reform)this.all_purpose).Parent.ToString() + "-" + ((RadioButton_reform)this.all_purpose).Name);//调用加载数据类                
        }

        private void Modification_RadioButton_Shown(object sender, EventArgs e)
        {
            //查询数据库是否有该数据
            if (Button_EFbase.Button_Parameter_inquire<RadioButton_Class>(this.skinTextBox8.Text) == "OK")
            {
                Button_EFbase button_EF = new Button_EFbase();//实例化EF对象
                button = button_EF.Button_Parameter_Query<RadioButton_Class>(this.skinTextBox8.Text);//获取按钮类全部参数
                List_Index();//开始改变索引
            ...
            {
                //新增控件获取位置与大小
                this.skinTextBox1.Text = ((RadioButton_reform)this.all_purpose).Location.X.ToString();//控件位置X轴
                this.skinTextBox2.Text = ((RadioButton_reform)this.all_purpose).Location.Y.ToString();//控件位置Y轴
                this.skinTextBox4.Text = ((RadioButton_reform)this.all_purpose).Size.Width.ToString();//控件大小宽度
                this.skinTextBox3.Text = ((RadioButton_reform)this.all_purpose).Size.Height.ToString();//控件大小宽度
                if (skinRadioButton1.Checked == false & skinRadioButton4.Checked == false) skinRadioButton1.Checked = true;
        ...
        private void List_Index()
        {
            ...
            ((RadioButton_reform)this.all_purpose).Text = button.Control_state_0_content.Trim();
            ...
            this.skinTextBox3.Text = point_or_Size(button.size)[1].ToString();//控件大小宽度
                                                                              //新增按钮样式功能
            indexes.Button_ComboBoxIndex_fill(button.colour_0, ref this.colorComboBox1);
            Modification_label_parameter.label_Lists[button.Control_state_1].colour_1 = indexes.Button_ComboBox_Index(button.colour_1, this.colorComboBox1);
        }
        ...
        private void skinButton2_Click(object sender, EventArgs e)
        {
            ...
            if (Button_EFbase.Button_Parameter_inquire<RadioButton_Class>(this.skinTextBox8.Text) == "OK")
            {
                ...
                button_EF.Button_Parameter_modification(this.skinTextBox8.Text, button_Parameter(), tag_Common_Parameters(), general_Parameters_Of_Picture(), control_Location(), Button_colour_Location());//修改数据库参数
            }
            else
            {
                //LogUtils日志
                ...
                button_EF.Button_Parameter_Add(Button_colour_Location());//插入按钮样式
            }
            ((RadioButton_reform)this.all_purpose).Text = this.skinChatRichTextBox1.Text.Trim();//设置文本
            Public_attributeCalss public_AttributeCalss = new Public_attributeCalss();//实例化按钮参数设置
            public_AttributeCalss.attributeCalss((RadioButton_reform)this.all_purpose, button_EF.Button_Parameter_Query<RadioButton_Class>(skinTextBox8.Text.Trim()));//查询数据库--进行设置后的参数修改
        ...
        private void Modification_RadioButton_FormClosing(object sender, FormClosingEventArgs e)
        {
            AnimateWindow(this.Handle, 1000, AW_SLIDE | AW_ACTIVE | AW_VER_NEGATIVE);
        }
        private RadioButton_parameter button_Parameter()//获取要写入的一般参数
        {
            return new RadioButton_parameter
            {
        ...
        private Tag_common_parameters tag_Common_Parameters()//获取要写入的标签参数
        {
            ...
            if (Convert.ToInt32(this.skinComboBox2.Text) == 1)//判断当前用户选中的格式状态
            {
                ...
                tag_Common = new Tag_common_parameters
                {
                    ...
                    Control_type = ((RadioButton_reform)this.all_purpose).Name,
                    FROM = From_Name,
        ...
        {
            Button_colour Button_colour = new Button_colour();//定义要返回的对象
            int Index = 0;//定义索引的指针
            if (Convert.ToInt32(this.skinComboBox2.Text) == 1)//判断当前用户选中的格式状态
            {
                Index = 0;//定义索引的指针
                Button_colour = new Button_colour
                {
                    FORM = From_Name,
                    ID = skinTextBox8.Text,
                    colour_0 = this.colorComboBox1.Items[Modification_label_parameter.label_Lists[Index].colour_1].ToString(),
                    colour_1 = this.colorComboBox1.Text
                };
            }
            if (Convert.ToInt32(this.skinComboBox2.Text) == 0)//判断当前用户选中的格式状态
            {
                ...
                Button_colour = new Button_colour
                {
                    FORM = From_Name,
                    ID = skinTextBox8.Text,
                    colour_1 = this.colorComboBox1.Items[Modification_label_parameter.label_Lists[Index].colour_1].ToString(),
                    colour_0 = this.colorComboBox1.Text
                };
            }
