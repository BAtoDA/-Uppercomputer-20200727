...
namespace 自定义Uppercomputer_20200727.修改参数界面.doughnut_Chart图形控件参数
{
    public partial class Modification_doughnut_Chart : Skin_VS
    {
        ...
        private void Modification_doughnut_Chart_Shown(object sender, EventArgs e)
        {
            ...
            //查询数据库是否有该数据
            if (Button_EFbase.Button_Parameter_inquire<doughnut_Chart_Class>(this.skinTextBox8.Text) == "OK")
            {
                Button_EFbase doughnut_Chart_EF = new Button_EFbase();//实例化EF对象
                button = doughnut_Chart_EF.Button_Parameter_Query<doughnut_Chart_Class>(this.skinTextBox8.Text);//获取按钮类全部参数
                List_Index();//开始改变索引
            }
            else
            {
        ...
        private void skinButton2_Click(object sender, EventArgs e)
        {
            ...
            public_AttributeCalss.attributeCalss((doughnut_Chart_reform)this.all_purpose, numerical_EF.Button_Parameter_Query<doughnut_Chart_Class>(this.skinTextBox8.Text));//查询数据库--进行设置后的参数修改
            Add_to_allow = true;
            this.Close();
            this.Dispose();
        }
