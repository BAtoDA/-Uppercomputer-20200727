...
{
    public partial class Modification_HScrollBar : Skin_VS
    {
        private string Button_ID;//控件的基本信息
        private object all_purpose;//通用类型
        public bool Add_to_allow = false;//反馈标志位是否允许添加
        HScrollBar_Class button;//按钮类全部参数
        private string From_Name;//控件处在的窗口名称
        public Modification_HScrollBar(string ID, object all_purpose)
        {
            InitializeComponent();
            this.Button_ID = ID;//获取参数
            this.all_purpose = all_purpose;
            this.skinTextBox8.Text = Button_ID.Trim() + "- " + ((HScrollBar_reform)all_purpose).Name;//保存ID 为一地址  -是分割字符 
        ...
        #endregion
        private void Modification_HScrollBar_Load(object sender, EventArgs e)
        {
            ...
            {this.skinTabPage1, this.skinTabPage2, this.skinTabPage3, this.skinTabPage4,this.skinTabPage5,this.skinTabPage6}, ((HScrollBar_reform)all_purpose).Name);
        }

        private void Modification_HScrollBar_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void Modification_HScrollBar_Shown(object sender, EventArgs e)
        {
            ...
            {
                //新增控件获取位置与大小
                this.textBox2.Text = ((HScrollBar_reform)this.all_purpose).Location.X.ToString();//控件位置X轴
                this.textBox1.Text = ((HScrollBar_reform)this.all_purpose).Location.Y.ToString();//控件位置Y轴
                this.skinTextBox4.Text = ((HScrollBar_reform)this.all_purpose).Size.Width.ToString();//控件大小宽度
                this.skinTextBox3.Text = ((HScrollBar_reform)this.all_purpose).Size.Height.ToString();//控件大小宽度
                //初始化属性
        ...
        private HScrollBar_parameter numerical_Parameter()//获取要写入的一般参数
        {
            return new HScrollBar_parameter
            {
        ...
        private Tag_common_parameters tag_Common_Parameters()//获取要写入的标签参数
        {
            return new Tag_common_parameters
            {
                ...
                Control_state_0_content = skinChatRichTextBox1.Text ?? ((HScrollBar_reform)this.all_purpose).Name,
                //写入1状态
                Control_state_1 = 1,
                ...
                Control_type = ((HScrollBar_reform)this.all_purpose).Name,
                FROM = From_Name,
