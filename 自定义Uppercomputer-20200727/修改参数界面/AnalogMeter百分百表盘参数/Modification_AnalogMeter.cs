...
{
    public partial class Modification_AnalogMeter :Skin_VS
    {
        private string Button_ID;//控件的基本信息
        private object all_purpose;//通用类型
        public bool Add_to_allow = false;//反馈标志位是否允许添加
        AnalogMeter_Class button;//按钮类全部参数
        private string From_Name;//控件处在的窗口名称
        public Modification_AnalogMeter(string ID, object all_purpose)
        {
            InitializeComponent();
            this.Button_ID = ID;//获取参数
            this.all_purpose = all_purpose;
            this.skinTextBox8.Text = Button_ID.Trim() + "- " + ((AnalogMeter_reform)all_purpose).Name;//保存ID 为一地址  -是分割字符 
        ...
        #endregion
        private void Modification_AnalogMeter_Load(object sender, EventArgs e)
        {
            ...
            {this.skinTabPage1, this.skinTabPage2, this.skinTabPage3, this.skinTabPage4,this.skinTabPage5,this.skinTabPage6}, ((AnalogMeter_reform)all_purpose).Name);
        }
        ...
        private void Modification_AnalogMeter_Shown(object sender, EventArgs e)
        {
            ...
            {
                //新增控件获取位置与大小
                this.textBox2.Text = ((AnalogMeter_reform)this.all_purpose).Location.X.ToString();//控件位置X轴
                this.textBox1.Text = ((AnalogMeter_reform)this.all_purpose).Location.Y.ToString();//控件位置Y轴
                this.skinTextBox4.Text = ((AnalogMeter_reform)this.all_purpose).Size.Width.ToString();//控件大小宽度
                this.skinTextBox3.Text = ((AnalogMeter_reform)this.all_purpose).Size.Height.ToString();//控件大小宽度
                //初始化属性
        ...
        private void Modification_AnalogMeter_FormClosing(object sender, FormClosingEventArgs e)
        {
            AnimateWindow(this.Handle, 1000, AW_SLIDE | AW_ACTIVE | AW_VER_NEGATIVE);
        }
        private AnalogMeter_parameter numerical_Parameter()//获取要写入的一般参数
        {
            return new AnalogMeter_parameter
            {
        ...
        private Tag_common_parameters tag_Common_Parameters()//获取要写入的标签参数
        {
            return new Tag_common_parameters
            {
                ...
                Control_state_0_content = skinChatRichTextBox1.Text ?? ((AnalogMeter_reform)this.all_purpose).Name,
                //写入1状态
                Control_state_1 = 1,
                ...
                Control_type = ((AnalogMeter_reform)this.all_purpose).Name,
                FROM = From_Name,
