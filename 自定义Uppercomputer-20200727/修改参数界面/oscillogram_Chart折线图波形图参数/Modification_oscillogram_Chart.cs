...
{
    public partial class Modification_oscillogram_Chart : Skin_VS
    {
        private string Button_ID;//控件的基本信息
        private object all_purpose;//通用类型
        public bool Add_to_allow = false;//反馈标志位是否允许添加
        oscillogram_Chart_Class button;//按钮类全部参数
        private string From_Name;//控件处在的窗口名称
        List<string> oscillogram_Chart_Name = new List<string>();//名称索引
        public Modification_oscillogram_Chart(string ID, object all_purpose)
        {
            InitializeComponent();
            this.Button_ID = ID;//获取参数
            this.all_purpose = all_purpose;
            this.skinTextBox8.Text = Button_ID.Trim() + "- " + ((oscillogram_Chart_reform)all_purpose).Name;//保存ID 为一地址  -是分割字符 
        ...
        private void Modification_oscillogram_Chart_Shown(object sender, EventArgs e)
        {
            //设置默认背景颜色
            this.colorComboBox2.SelectedIndex = 0;//默认透明
            this.colorComboBox2.SelectedItem = 0;//默认透明
            //查询数据库是否有该数据
            if (oscillogram_Chart_EF.oscillogram_Chart_Parameter_inquire(this.skinTextBox8.Text) == "OK")
            {
                oscillogram_Chart_EF oscillogram_Chart_EF = new oscillogram_Chart_EF();//实例化EF对象
                button = oscillogram_Chart_EF.oscillogram_Chart_Parameter_Query(this.skinTextBox8.Text);//获取按钮类全部参数
                List_Index();//开始改变索引
            }
            else
            {
                //新增控件获取位置与大小
                this.textBox2.Text = ((oscillogram_Chart_reform)this.all_purpose).Location.X.ToString();//控件位置X轴
                this.textBox1.Text = ((oscillogram_Chart_reform)this.all_purpose).Location.Y.ToString();//控件位置Y轴
                this.skinTextBox4.Text = ((oscillogram_Chart_reform)this.all_purpose).Size.Width.ToString();//控件大小宽度
                this.skinTextBox3.Text = ((oscillogram_Chart_reform)this.all_purpose).Size.Height.ToString();//控件大小宽度
                //初始化属性
                this.skinTextBox5.Text = Convert.ToString(0);
                this.skinTextBox9.Text = Convert.ToString(100);
                this.skinTextBox10.Text = Convert.ToString(5);
        ...
        private void List_Index()
        {
            ...
            //控件参数
            this.skinTextBox5.Text = button.Min.ToString();
            this.skinTextBox9.Text = button.Max.ToString();
            this.skinTextBox10.Text = button.刷新时间.ToString();
            this.skinCheckBox2.Checked = button.折线图_曲线图.ToBool();
            //获取颜色设置
            indexes.Button_ComboBoxIndex_fill(button.colour_0.Trim(), ref this.colorComboBox2);
        }
        private void Modification_oscillogram_Chart_FormClosing(object sender, FormClosingEventArgs e)
        {
            AnimateWindow(this.Handle, 1000, AW_SLIDE | AW_ACTIVE | AW_VER_NEGATIVE);
        }
        ...
        private void skinButton2_Click(object sender, EventArgs e)
        {
            ...
            oscillogram_Chart_EF numerical_EF = new oscillogram_Chart_EF();//实例化EF对象
            if (oscillogram_Chart_EF.oscillogram_Chart_Parameter_inquire(this.skinTextBox8.Text) == "OK")
                numerical_EF.oscillogram_Chart_modification(this.skinTextBox8.Text, numerical_Parameter(), tag_Common_Parameters(), control_Location(), Button_colour_Location());//修改数据库参数
            else
            {
                numerical_EF.oscillogram_Chart_Parameter_Add(tag_Common_Parameters());//插入标签参数
                numerical_EF.oscillogram_Chart_Parameter_Add(numerical_Parameter());//插入一般参数
                numerical_EF.oscillogram_Chart_Parameter_Add(control_Location());//插入控件坐标参数
                numerical_EF.oscillogram_Chart_Parameter_Add(Button_colour_Location());//插入控件背景颜色
            }
            Public_attributeCalss public_AttributeCalss = new Public_attributeCalss();//实例化按钮参数设置
            ((oscillogram_Chart_reform)this.all_purpose).Chart_Minimum = this.skinTextBox5.Text.ToInt32();//设置最小值
            ((oscillogram_Chart_reform)this.all_purpose).Chart_Maximum = this.skinTextBox9.Text.ToInt32();//设置最大值
            ((oscillogram_Chart_reform)this.all_purpose).Chart_Interval = this.skinTextBox9.Text.ToInt32();//设置刷新时间
            ((oscillogram_Chart_reform)this.all_purpose).waveform_ON = this.skinCheckBox2.Checked;//获取用户设置
            public_AttributeCalss.attributeCalss((oscillogram_Chart_reform)this.all_purpose, numerical_EF.oscillogram_Chart_Parameter_Query(this.skinTextBox8.Text));//查询数据库--进行设置后的参数修改
            Add_to_allow = true;
            this.Close();
            this.Dispose();
        }
        private oscillogram_Chart_parameter numerical_Parameter()//获取要写入的一般参数
        {
            string DATA = " ";//创建空字符串
            return new oscillogram_Chart_parameter
            {
                ...
                Name_Text = DATA,
                Min = this.skinTextBox5.Text.ToInt32(),
                Max = this.skinTextBox9.Text.ToInt32(),
                刷新时间 = this.skinTextBox10.Text.ToInt32(),
                折线图_曲线图 = this.skinCheckBox2.Checked.ToInt32(),
                通道数量 = 0
            };
        }
        private Tag_common_parameters tag_Common_Parameters()//获取要写入的标签参数
        {
            return new Tag_common_parameters
            {
                ...
                Control_state_0_content = skinChatRichTextBox1.Text ?? ((oscillogram_Chart_reform)this.all_purpose).Name,
                //写入1状态
                Control_state_1 = 1,
                ...
                Control_type = ((oscillogram_Chart_reform)this.all_purpose).Name,
                FROM = From_Name,
