...
namespace 自定义Uppercomputer_20200727.修改参数界面.function_key功能键参数
{
    public partial class Modification_function_key : Skin_VS
    {

        private string Button_ID;//控件的基本信息
        private object all_purpose;//通用类型
        public bool Add_to_allow = false;//反馈标志位是否允许添加
        function_key_Class label;//按钮类全部参数
        private string From_Name;//控件处在的窗口名称
        public Modification_function_key(string ID, object all_purpose)
        {
            InitializeComponent();
            this.Button_ID = ID;//获取参数
            this.all_purpose = all_purpose;
            this.skinTextBox1.Text = Button_ID + "-" + ((function_key_reform)all_purpose).Name;//保存ID 为一地址  -是分割字符
            From_Name = parameter_indexes.Button_from_name(skinTextBox1.Text).Trim();//获取窗口名称
        }
        ...
        #endregion
        private void Modification_function_key_Load(object sender, EventArgs e)
        {
            AnimateWindow(this.Handle, 500, AW_SLIDE | AW_ACTIVE | AW_VER_NEGATIVE);
            Modification_label_Class _Label_Class = new Modification_label_Class(new List<SkinTabPage>() { this.skinTabPage1, this.skinTabPage2, this.skinTabPage3 }, ((function_key_reform)all_purpose).Name);//加载
        }

        private void Modification_function_key_Shown(object sender, EventArgs e)
        {
            //设置默认背景颜色
            parameter_indexes indexes = new parameter_indexes();//实例化索引器
            indexes.Button_ComboBoxIndex_fill("CornflowerBlue", ref this.colorComboBox2);
            //查询数据库是否有该数据
            if (function_key_EF.function_key_Parameter_inquire(this.skinTextBox1.Text) == "OK")
            {
                function_key_EF label_EF = new function_key_EF();//实例化EF对象
                label = label_EF.function_key_Parameter_Query(this.skinTextBox1.Text);//获取按钮类全部参数
                List_Index();//开始改变索引
            }
            else
            {
                //新增控件获取位置与大小
                this.textBox1.Text = ((function_key_reform)this.all_purpose).Location.X.ToString();//控件位置X轴
        ...
        private void skinButton2_Click(object sender, EventArgs e)
        {
            ...
            if (function_key_EF.function_key_Parameter_inquire(this.skinTextBox1.Text) == "OK")
            {
                ...
                function_key_EF.function_key_Parameter_modification(this.skinTextBox1.Text, Label_Parameter(), tag_Common_Parameters(), control_Location(), Button_colour_Location());//修改数据库参数
            }
            else
            {
                //LogUtils日志
                LogUtils.debugWrite($"用户向{((Control)all_purpose).Name} 控件插入参数");
                function_key_EF.function_key_Parameter_Add(Label_Parameter());//插入主参数
                function_key_EF.function_key_Parameter_Add(tag_Common_Parameters());//插入标签参数
                function_key_EF.function_key_Parameter_Add(control_Location());//插入控件坐标参数
                function_key_EF.function_key_Parameter_Add(Button_colour_Location());//插入控件背景颜色参数
            }
            Add_to_allow = true;
            ((function_key_reform)all_purpose).Text = this.skinChatRichTextBox1.Text;//修改显示内容
            Public_attributeCalss public_AttributeCalss = new Public_attributeCalss();//实例化按钮参数设置
            public_AttributeCalss.attributeCalss((function_key_reform)this.all_purpose, function_key_EF.function_key_Parameter_Query(this.skinTextBox1.Text.Trim()));//查询数据库--进行设置后的参数修改
            this.Close();
            this.Dispose();
        }
        private function_key_parameter Label_Parameter()//获取要写入的主参数
        {
        ...
        private Tag_common_parameters tag_Common_Parameters()//获取要写入的标签参数
        {
            return new Tag_common_parameters
            {
                ...
                Control_type = ((function_key_reform)this.all_purpose).Name,
                FROM = From_Name.Trim(),
                ID = skinTextBox1.Text
            };
        }
