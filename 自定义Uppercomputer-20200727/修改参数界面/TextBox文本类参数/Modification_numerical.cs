...
namespace 自定义Uppercomputer_20200727.修改参数界面
{
    public partial class Modification_numerical :Skin_VS
    {
        ...
        private void Modification_numerical_Shown(object sender, EventArgs e)
        {
            ...
            //查询数据库是否有该数据
            if (Button_EFbase.Button_Parameter_inquire<numerical_Class>(this.skinTextBox8.Text) == "OK")
            {
                Button_EFbase button_EF = new Button_EFbase();//实例化EF对象
                button = button_EF.Button_Parameter_Query<numerical_Class>(this.skinTextBox8.Text);//获取按钮类全部参数
                List_Index();//开始改变索引
            }
            else
            {
        ...
        private void skinButton2_Click(object sender, EventArgs e)//用户点击了保存
        {
            ...
            if (Button_EFbase.Button_Parameter_inquire<numerical_Class>(this.skinTextBox8.Text) == "OK")
            {
                ...
                numerical_EF.Button_Parameter_modification(this.skinTextBox8.Text, numerical_Parameter(), tag_Common_Parameters(), general_Parameters_Of_Picture(), control_Location(), Button_colour_Location());//修改数据库参数
            }
            else
            {
                //LogUtils日志
                ...
                numerical_EF.Button_Parameter_Add(general_Parameters_Of_Picture());//插入图片参数
            ...
            public_AttributeCalss.attributeCalss((SkinTextBox_reform)this.all_purpose, numerical_EF.Button_Parameter_Query<numerical_Class>(this.skinTextBox8.Text));//查询数据库--进行设置后的参数修改
            Add_to_allow = true;
            this.Close();
            this.Dispose();
        }
