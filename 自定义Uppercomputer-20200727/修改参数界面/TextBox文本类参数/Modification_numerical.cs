...
{
    public partial class Modification_numerical :Skin_VS
    {
        private string Button_ID;//控件的基本信息
        private object all_purpose;//通用类型
        public bool Add_to_allow = false;//反馈标志位是否允许添加
        numerical_Class button;//按钮类全部参数
        private string From_Name;//控件处在的窗口名称
        public Modification_numerical(string ID, object all_purpose)
        {
            InitializeComponent();
            this.Button_ID = ID;//获取参数
            this.all_purpose = all_purpose;
            this.skinTextBox8.Text = Button_ID.Trim() + "- " + ((SkinTextBox_reform)all_purpose).Name;//保存ID 为一地址  -是分割字符 
        ...
        #endregion
        private void Modification_numerical_Load(object sender, EventArgs e)//加载窗口
        {
            AnimateWindow(this.Handle, 1000, AW_SLIDE | AW_ACTIVE | AW_VER_NEGATIVE);
            Modification_numerical_Class numerical_Class = new Modification_numerical_Class(new List<SkinTabPage>()
            {this.skinTabPage1, this.skinTabPage2, this.skinTabPage3, this.skinTabPage4,this.skinTabPage5,this.skinTabPage6}, ((SkinTextBox_reform)all_purpose).Name);
        }

        private void Modification_numerical_FormClosing(object sender, FormClosingEventArgs e)
        {
            AnimateWindow(this.Handle, 1000, AW_SLIDE | AW_ACTIVE | AW_VER_NEGATIVE);
        }

        private void Modification_numerical_Shown(object sender, EventArgs e)
        {
            //设置默认背景颜色
            ...
            //查询数据库是否有该数据
            if (numerical_EF.numerical_Parameter_inquire(this.skinTextBox8.Text) == "OK")
            {
                numerical_EF button_EF = new numerical_EF();//实例化EF对象
                button = button_EF.numerical_Parameter_Query(this.skinTextBox8.Text);//获取按钮类全部参数
                List_Index();//开始改变索引
            }
            else
            {
                //新增控件获取位置与大小
                this.textBox2.Text = ((SkinTextBox_reform)this.all_purpose).Location.X.ToString();//控件位置X轴
                this.textBox1.Text = ((SkinTextBox_reform)this.all_purpose).Location.Y.ToString();//控件位置Y轴
                this.skinTextBox4.Text = ((SkinTextBox_reform)this.all_purpose).Size.Width.ToString();//控件大小宽度
                this.skinTextBox3.Text = ((SkinTextBox_reform)this.all_purpose).Size.Height.ToString();//控件大小宽度        
            }
        }
        ...
        private void skinButton2_Click(object sender, EventArgs e)//用户点击了保存
        {
            ...
            if (numerical_EF.numerical_Parameter_inquire(this.skinTextBox8.Text) == "OK")
                numerical_EF.numerical_Parameter_modification(this.skinTextBox8.Text, numerical_Parameter(), tag_Common_Parameters(), general_Parameters_Of_Picture(), control_Location(), Button_colour_Location());//修改数据库参数
            else
            {
                numerical_EF.numerical_Parameter_Add(tag_Common_Parameters());//插入标签参数
                numerical_EF.numerical_Parameter_Add(numerical_Parameter());//插入一般参数
                numerical_EF.numerical_Parameter_Add(general_Parameters_Of_Picture());//插入图片参数
                numerical_EF.numerical_Parameter_Add(control_Location());//插入控件坐标参数
            ...
            public_AttributeCalss.attributeCalss((SkinTextBox_reform)this.all_purpose, numerical_EF.numerical_Parameter_Query(this.skinTextBox8.Text));//查询数据库--进行设置后的参数修改
            Add_to_allow = true;
            this.Close();
            this.Dispose();
        }
        private numerical_parameter numerical_Parameter()//获取要写入的一般参数
        {
            return new numerical_parameter
            {
                ...
                读写不同地址_ON_OFF = this.skinCheckBox1.Checked.ToInt32(),
        ...
        private Tag_common_parameters tag_Common_Parameters()//获取要写入的标签参数
        {
                return new Tag_common_parameters
                {
                    ...
                    Control_state_0_content = skinChatRichTextBox1.Text??((SkinTextBox_reform)this.all_purpose).Name,
                    //写入1状态
                    Control_state_1 = 1,
                    ...
                    Control_type =((SkinTextBox_reform)this.all_purpose).Name,
                    FROM = From_Name,
