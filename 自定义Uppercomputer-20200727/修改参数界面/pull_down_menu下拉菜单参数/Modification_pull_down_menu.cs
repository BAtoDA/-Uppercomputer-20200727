...
{
    public partial class Modification_pull_down_menu : Skin_VS
    {
        private string pull_down_menu_ID;//控件的基本信息
        private object all_purpose;//通用类型
        public bool Add_to_allow = false;//反馈标志位是否允许添加
        pull_down_menu_Class button;//按钮类全部参数
        private string From_Name;//控件处在的窗口名称
        string[] project_Name = new string[] { "预设" };//目前就开放一个
        //定义一个BindingList的学生链表
        private BindingList<pull_down_menuName> BStudent;
        //预设一个数据表最大100组
        private BindingList<pull_down_menuName> preinstall = new BindingList<pull_down_menuName>();
        public Modification_pull_down_menu(string ID, object all_purpose)
        {
            InitializeComponent();
            this.pull_down_menu_ID = ID;//获取参数
            this.all_purpose = all_purpose;
            skinTextBox8.Text = pull_down_menu_ID + "-" + ((pull_down_menu_reform)this.all_purpose).Name;//保存ID 为一地址  -是分割字符 
        ...
        private void Modification_pull_down_menu_FormClosing(object sender, FormClosingEventArgs e)
        {
            AnimateWindow(this.Handle, 1000, AW_SLIDE | AW_ACTIVE | AW_VER_NEGATIVE);
        }

        private void Modification_pull_down_menu_Shown(object sender, EventArgs e)
        {
            //设置默认背景颜色
            ...
            //设置默认背景颜色
            this.colorComboBox3.Items.RemoveAt(0);//移除透明背景颜色
            this.colorComboBox3.SelectedIndex = 0;
            this.colorComboBox3.SelectedItem = 0;
            //设置项目来源
            foreach (String i in project_Name)
                this.skinComboBox2.Items.Add(i);
            for (int i = 1; i < 101; i++)
                this.skinComboBox4.Items.Add(i.ToString());
            this.skinComboBox2.SelectedIndex = 0;
            this.skinComboBox2.SelectedItem = 0;
            this.skinComboBox4.SelectedIndex = 0;
            this.skinComboBox4.SelectedItem = 0;
            skinComboBox4_load();
            //查询数据库是否有该数据
            if (pull_down_menu_EF.pull_down_menu_Parameter_inquire(this.skinTextBox8.Text) == "OK")
            {
                pull_down_menu_EF button_EF = new pull_down_menu_EF();//实例化EF对象
                button = button_EF.pull_down_menu_Parameter_Query(this.skinTextBox8.Text);//获取按钮类全部参数
                List_Index();//开始改变索引
            }
            else
            {
                //新增控件获取位置与大小
                this.textBox2.Text = ((pull_down_menu_reform)this.all_purpose).Location.X.ToString();//控件位置X轴
                this.textBox1.Text = ((pull_down_menu_reform)this.all_purpose).Location.Y.ToString();//控件位置Y轴
                this.skinTextBox4.Text = ((pull_down_menu_reform)this.all_purpose).Size.Width.ToString();//控件大小宽度
                this.skinTextBox3.Text = ((pull_down_menu_reform)this.all_purpose).Size.Height.ToString();//控件大小宽度        
            }
        }
        private void List_Index()
        {
            ...
            this.skinTextBox3.Text = point_or_Size(button.size)[1].ToString();//控件大小宽度
                                                                              //获取颜色设置
            indexes.Button_ComboBoxIndex_fill(button.下拉背景.Trim(), ref this.colorComboBox2);
            indexes.Button_ComboBoxIndex_fill(button.选择背景.Trim(), ref this.colorComboBox3);
            this.skinComboBox2.SelectedIndex = button.项目资料来源;
            this.skinComboBox2.SelectedItem = button.项目资料来源;
            this.skinComboBox4.SelectedIndex = button.项目数量-1;
            this.skinComboBox4.SelectedItem = button.项目数量-1;
            //查询项目的数据
            //将IList中的值赋给对应的BindingList
            this.preinstall = new BindingList<pull_down_menuName>(pull_down_menu_EF.pull_down_menu_inquire(skinTextBox8.Text));//查询数据库获得数据
            this.skinDataGridView1.DataSource = preinstall;//绑定数据源
            //屏蔽一些数据--不允许用户更改
            this.skinDataGridView1.Columns[0].Visible = false;
            this.skinDataGridView1.Columns[1].Visible = false;
            this.skinDataGridView1.Columns[5].Visible = false;
        }
        ...
        private void skinButton2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                pull_down_menu_EF.pull_down_menu_Parameter_Add(control_Location());//插入控件坐标参数
                pull_down_menu_EF.pull_down_menu_Parameter_Add(pull_down_menuName());//插入控件坐标参数
            }
            Public_attributeCalss public_AttributeCalss = new Public_attributeCalss();//实例化按钮参数设置
            if (((pull_down_menu_reform)this.all_purpose).Items.Count > 0) ((pull_down_menu_reform)this.all_purpose).Items.Clear();
            for (int i = 0; i < preinstall.Count; i++)
            {
                ((pull_down_menu_reform)this.all_purpose).Items.Add(preinstall[i].项目资料.Trim());//把数据添加到控件
            }
            ((pull_down_menu_reform)this.all_purpose).SelectedIndex=0;//默认索引
            ((pull_down_menu_reform)this.all_purpose).SelectedItem=0;//默认索引
            public_AttributeCalss.attributeCalss((pull_down_menu_reform)this.all_purpose, pull_down_menu_EF.pull_down_menu_Parameter_Query(this.skinTextBox8.Text));//查询数据库--进行设置后的参数修改
            Add_to_allow = true;
            this.Close();
            this.Dispose();
        }
        private pull_down_menu_parameter numerical_Parameter()//获取要写入的一般参数
        {
            return new pull_down_menu_parameter
            {
                ...
                下拉背景 = colorComboBox2.Text,
                选择背景 = colorComboBox3.Text,
                操作模式 = "1",
                位切换开关 = "1",
                位指示灯 = "1",
                模式 = 1,
                项目数量 = Convert.ToInt32(skinComboBox4.Text ?? "1"),
                项目资料来源 = 0
        ...
        private Tag_common_parameters tag_Common_Parameters()//获取要写入的标签参数
        {
            return new Tag_common_parameters
            {
                ...
                Control_state_0_content = skinChatRichTextBox1.Text ?? ((pull_down_menu_reform)this.all_purpose).Name,
                //写入1状态
                Control_state_1 = 1,
                ...
                Control_type = ((pull_down_menu_reform)this.all_purpose).Name,
                FROM = From_Name,
        ...
        private List<pull_down_menuName> pull_down_menuName()//获取要写入的控件位置
        {
            return preinstall.ToList();
        }
        ...
        private void skinComboBox4_load()//选择下拉菜单
        {
            preinstall.Clear();//清空集合
            for (int i =0; i < this.skinComboBox4.SelectedIndex + 1; i++)
            {
                preinstall.Add(new EF实体模型.pull_down_menuName()
                {
                    ID = skinTextBox8.Text + i,
                    FORM = From_Name,
                    控件归属 = skinTextBox8.Text
                ,
                    项目 = i,
                    数据 = i.ToString(),
                    项目资料 = "PLC_" + i
            ...
            this.skinDataGridView1.DataSource = preinstall;//重新绑定数据源
            //屏蔽一些数据--不允许用户更改
            this.skinDataGridView1.Columns[0].Visible = false;
            this.skinDataGridView1.Columns[1].Visible = false;
            this.skinDataGridView1.Columns[5].Visible = false;
        ...
        private void skinComboBox4_SelectionChangeCommitted(object sender, EventArgs e)
        {
            skinComboBox4_load();//加载数据
        }
        private void skinDataGridView1_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.RowIndex > -1)//判断用户是否选中行
            {
                ...
                {
                    MessageBox.Show("你点击了空行");
                }
                else
                {
                    pull_down_menuName modifySt = this.skinDataGridView1.CurrentRow.DataBoundItem as pull_down_menuName;
                    int index = this.skinDataGridView1.CurrentRow.Index;
                    Project_Parameters project_Parameters = new Project_Parameters(preinstall[index].项目.ToString(), preinstall[index].数据, preinstall[index].项目资料);//实例化
                    project_Parameters.ShowDialog();
                    if (project_Parameters.operation != true) return;
                    modifySt.项目 = project_Parameters.ID.ToInt32();
                    modifySt.数据 = project_Parameters.Data;
                    modifySt.项目资料 = project_Parameters.Name;
                    this.preinstall.RemoveAt(index);//先移除当前行
                    this.preinstall.Insert(index, modifySt);//添加修改后的行
                }
            }
        }
