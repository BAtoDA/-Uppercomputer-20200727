...
{
    public partial class Modification_Ledbulb : Skin_VS
    {
        private string Button_ID;//控件的基本信息
        private object all_purpose;//通用类型
        public bool Add_to_allow = false;//反馈标志位是否允许添加
        LedBulb_Class button;//切换开关类全部参数
        private string From_Name;//控件处在的窗口名称
        public Modification_Ledbulb(string ID, object all_purpose)
        {
            InitializeComponent();
            this.Button_ID = ID;//获取参数
            this.all_purpose = all_purpose;
            skinTextBox8.Text = Button_ID + "-" + ((LedBulb_reform)this.all_purpose).Name;//保存ID 为一地址  -是分割字符 
        ...
        private void Modification_Ledbulb_Shown(object sender, EventArgs e)
        {
            ...
            {
                //新增控件获取位置与大小
                this.skinTextBox1.Text = ((LedBulb_reform)this.all_purpose).Location.X.ToString();//控件位置X轴
                this.skinTextBox2.Text = ((LedBulb_reform)this.all_purpose).Location.Y.ToString();//控件位置Y轴
                this.skinTextBox4.Text = ((LedBulb_reform)this.all_purpose).Size.Width.ToString();//控件大小宽度
                this.skinTextBox3.Text = ((LedBulb_reform)this.all_purpose).Size.Height.ToString();//控件大小宽度
            }
        }
        ...
        private void Modification_Ledbulb_Load(object sender, EventArgs e)
        {
           ...
           (new List<SkinTabPage> { this.skinTabPage1, this.skinTabPage2, this.skinTabPage3, this.skinTabPage4, this.skinTabPage5 }, ((LedBulb_reform)this.all_purpose).Name, ((LedBulb_reform)this.all_purpose).Parent.ToString() + "-" + ((LedBulb_reform)this.all_purpose).Name);//调用加载数据类                
        }   
        #region 窗体关闭效果
        ...
        private void Modification_Ledbulb_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void List_Index()
        {
            ...
            ((LedBulb_reform)this.all_purpose).Text = button.Control_state_0_content.Trim();
            ...
            this.skinTextBox3.Text = point_or_Size(button.size)[1].ToString();//控件大小宽度
               //新增按钮样式功能
            indexes.Button_ComboBoxIndex_fill(button.colour_0.Trim(), ref this.colorComboBox1);
            Modification_label_parameter.label_Lists[button.Control_state_1].colour_1 = indexes.Button_ComboBox_Index(button.colour_1.Trim(), this.colorComboBox1);
        }
        ...
        private LedBulb_parameter button_Parameter()//获取要写入的一般参数
        {
            return new LedBulb_parameter
            {
        ...
        private Tag_common_parameters tag_Common_Parameters()//获取要写入的标签参数
        {
            ...
            if (Convert.ToInt32(this.skinComboBox2.Text) == 1)//判断当前用户选中的格式状态
            {
                ...
                tag_Common = new Tag_common_parameters
                {
                    ...
                    Control_type = ((LedBulb_reform)this.all_purpose).Name,
                    FROM = From_Name,
        ...
        private Button_colour Button_colour_Location()//获取要写入的控件位置
        {
