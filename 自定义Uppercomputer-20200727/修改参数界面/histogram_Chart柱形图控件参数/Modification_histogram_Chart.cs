...
{
    public partial class Modification_histogram_Chart : Skin_VS
    {
        private string Button_ID;//控件的基本信息
        private object all_purpose;//通用类型
        public bool Add_to_allow = false;//反馈标志位是否允许添加
        histogram_Chart_Class button;//按钮类全部参数
        private string From_Name;//控件处在的窗口名称
        string[] histogram_Chart_Name;//名称索引
        public Modification_histogram_Chart(string ID, object all_purpose)
        {
            InitializeComponent();
            this.Button_ID = ID;//获取参数
            this.all_purpose = all_purpose;
            this.skinTextBox8.Text = Button_ID.Trim() + "- " + ((histogram_Chart_reform)all_purpose).Name;//保存ID 为一地址  -是分割字符 
            From_Name = parameter_indexes.Button_from_name(skinTextBox8.Text);//获取窗口名称
            this.histogram_Chart_Name = ((histogram_Chart_reform)all_purpose).x;//获取默认名称
        }
        ...
        #endregion
        private void Modification_histogram_Chart_Load(object sender, EventArgs e)
        {
            AnimateWindow(this.Handle, 1000, AW_SLIDE | AW_ACTIVE | AW_VER_NEGATIVE);
            Modification_numerical_Class numerical_Class = new Modification_numerical_Class(new List<SkinTabPage>()
            {this.skinTabPage1, this.skinTabPage2, this.skinTabPage3, this.skinTabPage4,this.skinTabPage5,this.skinTabPage6}, ((histogram_Chart_reform)all_purpose).Name);
            ...
            skinTextBox5.Text = histogram_Chart_Name[skinComboBox4.SelectedIndex];//名称
            skinTextBox9.Text= histogram_Chart_Name[skinComboBox4.SelectedIndex+1];//比较值名称
        }

        private void Modification_histogram_Chart_FormClosing(object sender, FormClosingEventArgs e)
        {
            AnimateWindow(this.Handle, 1000, AW_SLIDE | AW_ACTIVE | AW_VER_NEGATIVE);
        }
        private void Modification_histogram_Chart_Shown(object sender, EventArgs e)
        {
            //设置默认背景颜色
            ...
            {
                //新增控件获取位置与大小
                this.textBox2.Text = ((histogram_Chart_reform)this.all_purpose).Location.X.ToString();//控件位置X轴
                this.textBox1.Text = ((histogram_Chart_reform)this.all_purpose).Location.Y.ToString();//控件位置Y轴
                this.skinTextBox4.Text = ((histogram_Chart_reform)this.all_purpose).Size.Width.ToString();//控件大小宽度
                this.skinTextBox3.Text = ((histogram_Chart_reform)this.all_purpose).Size.Height.ToString();//控件大小宽度
            }
        }
        private void List_Index()
        {
            ...
            //获取用户设定的通道数量与名称
            histogram_Chart_Name = point_or_Name(button.Name_Text).ToArray();//获取数据库设置名称
            indexes.Button_ComboBoxIndex_fill(button.通道数量.ToString(), ref this.skinComboBox4);
            skinTextBox5.Text = histogram_Chart_Name[button.通道数量*2];//名称
            skinTextBox9.Text = histogram_Chart_Name[(button.通道数量 *2)+1];//比较值名称
             //获取颜色设置
        ...
        private histogram_Chart_parameter numerical_Parameter()//获取要写入的一般参数
        {
            string DATA = "";//创建空字符串
            foreach (var i in histogram_Chart_Name) DATA += i.Trim() + "-";//获取用户设置的名称
            return new histogram_Chart_parameter
            {
        ...
        private Tag_common_parameters tag_Common_Parameters()//获取要写入的标签参数
        {
            return new Tag_common_parameters
            {
                ...
                Control_state_0_content = skinChatRichTextBox1.Text ?? ((histogram_Chart_reform)this.all_purpose).Name,
                //写入1状态
                Control_state_1 = 1,
                ...
                Control_type = ((histogram_Chart_reform)this.all_purpose).Name,
                FROM = From_Name,
        ...
        private void skinComboBox4_DropDown_1(object sender, EventArgs e)//下拉事件 进行数据名称保存
        {
            histogram_Chart_Name[skinComboBox4.SelectedIndex*2] = skinTextBox5.Text ?? "数据null";
            histogram_Chart_Name[(skinComboBox4.SelectedIndex*2 )+ 1] = skinTextBox9.Text ?? "数据null";
        }

        private void skinComboBox4_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            skinTextBox5.Text = histogram_Chart_Name[skinComboBox4.SelectedIndex*2];
            skinTextBox9.Text = histogram_Chart_Name[(skinComboBox4.SelectedIndex*2)+1];
        }
    }
}
