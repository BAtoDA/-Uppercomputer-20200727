...
{
    public partial class Form2derma1 : CCWin.Skin_DevExpress,FormIdentification
    {
        //1.声明自适应类实例
        ...
        private void navigationBar1_OptionitemClick(object sender, EventArgs e)
        {
            ...
            {
                int PageIndex = 0;
                for (int i=0;i< Windowclass.FromnamTexe.Length;i++)
                {
                    if (Windowclass.FromnamTexe[i] == (sender as Option).LabelText)
                    {
                        PageIndex = i;
                        break;
                    }
                }
                if (Windowclass.FromnamTexe[PageIndex] == this.Text || IsfunctionKey == false) return;
                string Data = this.Text;
                using (Windowclass windowclass = new Windowclass(this, new Form[] {new Form3(), new Form4(),new Form5()
                , new Form6(),new Form7(), new Form8(), new Form9()}, PageIndex))
                {
                }
            }
        }
