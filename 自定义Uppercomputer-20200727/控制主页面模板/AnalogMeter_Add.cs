...
namespace 自定义Uppercomputer_20200727.控制主页面模板
{
    ...
    /// 百分百表盘
    /// </summary>
    class AnalogMeter_Add
    {
        string Button_serial = "AnalogMeter_reform";//默认名称
        public AnalogMeter_reform Add(System.Windows.Forms.Control.ControlCollection control, Point point)//添加按钮方法
        {
            _ = control.Owner.Name;//获取创建的窗口名称
            this.Button_serial = Button_Name(control);//获得名称
            AnalogMeter_reform reform = new AnalogMeter_reform();//实例化按钮
        ...
        {
            List<System.Windows.Forms.Control> Data = (from Control pi in control where pi is AnalogMeter_reform select pi).ToList();//获得名称            
            int dex = 0;//获得序列
        inedx:
            dex += 1;
            string Name = Button_serial + (Data.Count + dex);//先定义名称
            foreach (AnalogMeter_reform i in Data)//遍历窗口是否有该名称存在
            {
