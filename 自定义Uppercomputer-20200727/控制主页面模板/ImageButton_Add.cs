...
namespace 自定义Uppercomputer_20200727.控制主页面模板
{
    class ImageButton_Add
    {
        ...
        {
            List<System.Windows.Forms.Control> Data = (from Control pi in control where pi is ImageButton_reform select pi).ToList();//获得名称            
            int dex = 0;//获得序列
        inedx:
            dex += 1;
            string Name = Button_serial + (Data.Count + dex);//先定义名称
            foreach (ImageButton_reform i in Data)//遍历窗口是否有该名称存在
            {
