...
namespace 自定义Uppercomputer_20200727.EF实体模型
{
    ...
    class Button_EF
    {
        public static string Button_Parameter_inquire(string ID)//按钮类参数ID查询
        {
            ...
            {
                Button_Class button_Parameter = model.Button_Class.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID
        ...
        public string Button_Parameter_Add(Button_parameter parameter)//按钮类主参数参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                Button_parameter button_Parameter = model.Button_parameter.Where(pi => pi.ID == parameter.ID).FirstOrDefault();//查询数据库是否有该ID
                if (button_Parameter == null)
                {
                    Button_parameter parameter1 = new Button_parameter();//实例化对象
                    parameter1 = parameter;//传入获取到的对象
                    model.Button_parameter.Add(parameter1);//构造添加表的SQL语句
                    model.SaveChanges();//执行操作
                }
            }
            return "操作成功";//返回
        }
        public string Button_Parameter_Add(Tag_common_parameters parameter)//按钮类字体参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
                {
        ...
        public string Button_Parameter_Add(General_parameters_of_picture parameter)//按钮类图片参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
        ...
        public string Button_Parameter_Add(control_location parameter)//按钮类位置坐标参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
        ...
        public Button_Class Button_Parameter_Query(string ID)//查询按钮类全部参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                Button_Class  button_Class = model.Button_Class.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID
                return button_Class;//返回查询结果
            }
        }

        public string Button_Parameter_delete(string ID)//按钮类ID删除参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                //执行删除按钮类主参数操作
                Button_parameter button_Parameter = model.Button_parameter.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID   
                if (button_Parameter != null)
                {
                    model.Button_parameter.Remove(button_Parameter);////构造添加表的SQL语句
                    model.SaveChanges();//执行操作
                }
                //执行删除按钮类字体参数操作
        ...
        public string Button_Parameter_modification(string ID, Button_parameter _Parameter, Tag_common_parameters tag_Common_,
        ...
        public string Button_Parameter_modification(string ID, Button_colour _Location)//按钮类ID修改参数--按钮类颜色--样式
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                //执行修改按钮类坐标参数操作
                Button_colour button_control_location = model.Button_colour.Where(pi => pi.ID.Trim() == ID.Trim()).FirstOrDefault();//查询数据库是否有该ID 
                if (button_control_location != null)
                {
                    #region 要修改的属性
                    button_control_location.colour_0 = _Location.colour_0;//获取对象
                    button_control_location.colour_1 = _Location.colour_1;//获取对象
                    #endregion
                    model.SaveChanges();//执行操作
                }
                if (button_control_location != null)
