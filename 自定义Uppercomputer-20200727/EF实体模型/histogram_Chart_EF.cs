...
{
    /// <summary>
    /// histogram_Chart_reform控件--对数据库进行增删改查
    /// </summary>
    class histogram_Chart_EF
    {
        public static string histogram_Chart_Parameter_inquire(string ID)//柱形图类参数ID查询
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                histogram_Chart_Class doughnut_Chart_Class = model.histogram_Chart_Class.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID
        ...
        public histogram_Chart_Class histogram_Chart_Parameter_Query(string ID)//查询柱形图类全部参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                histogram_Chart_Class histogram_Chart_Class = model.histogram_Chart_Class.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID
                return histogram_Chart_Class;//返回查询结果
            }
        }
        public histogram_Chart_Class histogram_Chart_Parameter_Query(string ID, string From)//查询柱形图类全部参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                histogram_Chart_Class histogram_Chart_Class = model.histogram_Chart_Class.Where(pi => pi.ID == ID & pi.FORM == From).FirstOrDefault();//查询数据库是否有该ID
                return histogram_Chart_Class;//返回查询结果
            }
        }
        public string histogram_Chart_Parameter_Add(histogram_Chart_parameter parameter)//柱形图类主参数参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                histogram_Chart_parameter button_Parameter = model.histogram_Chart_parameter.Where(pi => pi.ID == parameter.ID).FirstOrDefault();//查询数据库是否有该ID
                if (button_Parameter == null)
                {
                    histogram_Chart_parameter parameter1 = new histogram_Chart_parameter();//实例化对象
                    parameter1 = parameter;//传入获取到的对象
                    model.histogram_Chart_parameter.Add(parameter1);//构造添加表的SQL语句
                    model.SaveChanges();//执行操作
                }
            }
            return "操作成功";//返回
        }
        public string histogram_Chart_Parameter_Add(Tag_common_parameters parameter)//柱形图类--主参数--字体参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
        ...
        public string histogram_Chart_Parameter_Add(control_location parameter)//柱形图类位置坐标参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
        ...
        public string histogram_Chart_Parameter_Add(Button_colour parameter)//柱形图类颜色参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
        ...
        public string histogram_Chart_Parameter_delete(string ID)//柱形图类ID删除参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                //执行删除按钮类主参数操作
                histogram_Chart_parameter histogram_Chart_Parameter = model.histogram_Chart_parameter.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID   
                if (histogram_Chart_Parameter != null)
                {
                    model.histogram_Chart_parameter.Remove(histogram_Chart_Parameter);////构造添加表的SQL语句
                    model.SaveChanges();//执行操作
                }
                //执行删除按钮类字体参数操作
        ...
        public string histogram_Chart_modification(string ID, histogram_Chart_parameter _Parameter, Tag_common_parameters tag_Common_, control_location _Location, Button_colour button_)//柱形图类ID修改参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {

                //执行修改按钮类主参数操作
                histogram_Chart_parameter histogram_parameter = model.histogram_Chart_parameter.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID                 
                #region 要修改的属性
