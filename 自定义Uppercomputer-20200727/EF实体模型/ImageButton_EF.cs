...
namespace 自定义Uppercomputer_20200727.EF实体模型
{
    class ImageButton_EF
    {
        public static string Button_Parameter_inquire(string ID)//按钮类参数ID查询
        {
            ...
            {
                ImageButton_Class button_Parameter = model.ImageButton_Class.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID
        ...
        public string Button_Parameter_Add(ImageButton_parameter parameter)//按钮类主参数参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                ImageButton_parameter button_Parameter = model.ImageButton_parameter.Where(pi => pi.ID == parameter.ID).FirstOrDefault();//查询数据库是否有该ID
                if (button_Parameter == null)
                {
                    ImageButton_parameter parameter1 = new ImageButton_parameter();//实例化对象
                    parameter1 = parameter;//传入获取到的对象
                    model.ImageButton_parameter.Add(parameter1);//构造添加表的SQL语句
        ...
        public ImageButton_Class Button_Parameter_Query(string ID)//查询按钮类全部参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                ImageButton_Class button_Class = model.ImageButton_Class.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID
        ...
        public string Button_Parameter_delete(string ID)//按钮类ID删除参数
        {
            ...
            {
                //执行删除按钮类主参数操作
                ImageButton_parameter button_Parameter = model.ImageButton_parameter.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID   
                if (button_Parameter != null)
                {
                    model.ImageButton_parameter.Remove(button_Parameter);////构造添加表的SQL语句
                    model.SaveChanges();//执行操作
                }
                //执行删除按钮类字体参数操作
        ...
        public string Button_Parameter_modification(string ID, ImageButton_parameter _Parameter, Tag_common_parameters tag_Common_,
            General_parameters_of_picture _Of_Picture, control_location _Location)//按钮类ID修改参数
        {
