...
{
    /// <本类用于处理numerical类进行数据查询修改等操作>   
    class numerical_EF
    {
        public static string numerical_Parameter_inquire(string ID)//数值软件类参数ID查询
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                numerical_Class numerical_Parameter = model.numerical_Class.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID
        ...
        public string numerical_Parameter_Add(numerical_parameter parameter)//数值软件类主参数参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                numerical_parameter button_Parameter = model.numerical_parameter.Where(pi => pi.ID == parameter.ID).FirstOrDefault();//查询数据库是否有该ID
                if (button_Parameter == null)
                {
                    numerical_parameter parameter1 = new numerical_parameter();//实例化对象
                    parameter1 = parameter;//传入获取到的对象
                    model.numerical_parameter.Add(parameter1);//构造添加表的SQL语句
                    model.SaveChanges();//执行操作
                }
            }
            return "操作成功";//返回
        }
        public string numerical_Parameter_Add(Tag_common_parameters parameter)//数值软件类字体参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
        ...
        public string numerical_Parameter_Add(General_parameters_of_picture parameter)//数值软件类图片参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
        ...
        public string numerical_Parameter_Add(control_location parameter)//数值软件类位置坐标参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
        ...
        public numerical_Class numerical_Parameter_Query(string ID)//查询数值软件类全部参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                numerical_Class numerical_Class = model.numerical_Class.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID
                return numerical_Class;//返回查询结果
            }
        }
        public numerical_Class Button_Parameter_Query(string ID, string From)//查询数值软件类全部参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                numerical_Class numerical_Class = model.numerical_Class.Where(pi => pi.ID == ID & pi.FORM == From).FirstOrDefault();//查询数据库是否有该ID
                return numerical_Class;//返回查询结果
            }
        }
        public string Button_Parameter_Add(Button_colour parameter)//文本类颜色参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
        ...
        public string numerical_Parameter_delete(string ID)//数值软件类ID删除参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                //执行删除按钮类主参数操作
                numerical_parameter numerical_Parameter = model.numerical_parameter.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID   
                if (numerical_Parameter != null)
                {
                    model.numerical_parameter.Remove(numerical_Parameter);////构造添加表的SQL语句
                    model.SaveChanges();//执行操作
                }
                //执行删除按钮类字体参数操作
        ...
        public string numerical_Parameter_modification(string ID, numerical_parameter _Parameter, Tag_common_parameters tag_Common_,
