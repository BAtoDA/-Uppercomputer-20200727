...
namespace 自定义Uppercomputer_20200727.EF实体模型
{
    ...
    /// <summary>
    /// HScrollBar移动图形对数据库增删改查
    /// </summary>
    class HScrollBar_EF
    {
        public static string HScrollBar_Parameter_inquire(string ID)//纵向移动图形类参数ID查询
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                HScrollBar_Class HScrollBar_Class = model.HScrollBar_Class.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID
                if (HScrollBar_Class != null)
                    return "OK";
                else
                    return "NG";
            }
        }
        public HScrollBar_Class AnalogMeter_Parameter_Query(string ID)//查询纵向移动图形类全部参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                HScrollBar_Class HScrollBar_Class = model.HScrollBar_Class.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID
                return HScrollBar_Class;//返回查询结果
            }
        }
        public HScrollBar_Class HScrollBar_Parameter_Query(string ID, string From)//查询纵向移动图形类全部参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                HScrollBar_Class HScrollBar_Class = model.HScrollBar_Class.Where(pi => pi.ID == ID & pi.FORM == From).FirstOrDefault();//查询数据库是否有该ID
                return HScrollBar_Class;//返回查询结果
            }
        }
        public string HScrollBar_Parameter_Add(HScrollBar_parameter parameter)//纵向移动图形类主参数参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                HScrollBar_parameter AnalogMeter_Parameter = model.HScrollBar_parameter.Where(pi => pi.ID == parameter.ID).FirstOrDefault();//查询数据库是否有该ID
                if (AnalogMeter_Parameter == null)
                {
                    HScrollBar_parameter parameter1 = new HScrollBar_parameter();//实例化对象
                    parameter1 = parameter;//传入获取到的对象
                    model.HScrollBar_parameter.Add(parameter1);//构造添加表的SQL语句
                    model.SaveChanges();//执行操作
                }
            }
            return "操作成功";//返回
        }
        public string HScrollBar_Parameter_Add(Tag_common_parameters parameter)//纵向移动图形类--主参数--字体参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
        ...
        public string HScrollBar_Parameter_Add(control_location parameter)//纵向移动图形类位置坐标参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
        ...
        public string HScrollBar_Parameter_delete(string ID)//纵向移动图形类ID删除参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                //执行删除按钮类主参数操作
                HScrollBar_parameter HScrollBar_Parameter = model.HScrollBar_parameter.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID   
                if (HScrollBar_Parameter != null)
                {
                    model.HScrollBar_parameter.Remove(HScrollBar_Parameter);////构造添加表的SQL语句
                    model.SaveChanges();//执行操作
                }
                //执行删除按钮类字体参数操作
        ...
        public string HScrollBar_modification(string ID, HScrollBar_parameter _Parameter, Tag_common_parameters tag_Common_, control_location _Location)//纵向移动图形类ID修改参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {

                //执行修改按钮类主参数操作
                HScrollBar_parameter histogram_parameter = model.HScrollBar_parameter.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID                 
                #region 要修改的属性
