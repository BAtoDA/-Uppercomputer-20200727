...
namespace 自定义Uppercomputer_20200727.EF实体模型
{
    ...
    class LedBulb_EF
    {
        public static string Button_Parameter_inquire(string ID)//指示灯类参数ID查询
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                LedBulb_Class Switch_Parameter = model.LedBulb_Class.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID
                if (Switch_Parameter != null)
                    return "OK";
                else
                    return "NG";
            }
        }
        public string Button_Parameter_Add(LedBulb_parameter parameter)//指示灯类主参数参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                LedBulb_parameter Switch_Parameter = model.LedBulb_parameter.Where(pi => pi.ID == parameter.ID).FirstOrDefault();//查询数据库是否有该ID
                if (Switch_Parameter == null)
                {
                    LedBulb_parameter parameter1 = new LedBulb_parameter();//实例化对象
                    parameter1 = parameter;//传入获取到的对象
                    model.LedBulb_parameter.Add(parameter1);//构造添加表的SQL语句
                    model.SaveChanges();//执行操作
                }
            }
            return "操作成功";//返回
        }
        public string Button_Parameter_Add(Tag_common_parameters parameter)//指示灯类字体参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
        ...
        public string Button_Parameter_Add(General_parameters_of_picture parameter)//指示灯类图片参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
        ...
        public string Button_Parameter_Add(control_location parameter)//指示灯类位置坐标参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
        ...
        public LedBulb_Class Button_Parameter_Query(string ID)//查询指示灯类全部参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                LedBulb_Class button_Class = model.LedBulb_Class.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID
                return button_Class;//返回查询结果
            }
        }

        public string Button_Parameter_delete(string ID)//指示灯类ID删除参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                //执行删除切换开关类主参数操作
                LedBulb_parameter Switch_Parameter = model.LedBulb_parameter.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID   
                if (Switch_Parameter != null)
                {
                    model.LedBulb_parameter.Remove(Switch_Parameter);////构造添加表的SQL语句
                    model.SaveChanges();//执行操作
                }
                //执行删除切换开关类字体参数操作
        ...
        public string Button_Parameter_modification(string ID, LedBulb_parameter _Parameter, Tag_common_parameters tag_Common_,
            General_parameters_of_picture _Of_Picture, control_location _Location, Button_colour button_)//指示灯类ID修改参数
        {

            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                //执行修改切换开关类主参数操作
                LedBulb_parameter LedBulb_parameter = model.LedBulb_parameter.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID
                if (LedBulb_parameter != null)
                {
                    #region 要修改的属性
                    LedBulb_parameter.位切换开关 = _Parameter.位切换开关;//获取对象
                    LedBulb_parameter.位指示灯 = _Parameter.位指示灯;//获取对象
                    LedBulb_parameter.写设备_地址_具体地址_复选 = _Parameter.写设备_地址_具体地址_复选;//获取对象
                    LedBulb_parameter.写设备_地址_复选 = _Parameter.写设备_地址_复选;//获取对象
                    LedBulb_parameter.写设备_复选 = _Parameter.写设备_复选;//获取对象
                    LedBulb_parameter.操作安全时间 = _Parameter.操作安全时间;//获取对象
                    LedBulb_parameter.操作模式 = _Parameter.操作模式;//获取对象
                    LedBulb_parameter.读写不同地址_ON_OFF = _Parameter.读写不同地址_ON_OFF;//获取对象
                    LedBulb_parameter.读写设备 = _Parameter.读写设备;//获取对象
                    LedBulb_parameter.读写设备_地址 = _Parameter.读写设备_地址;//获取对象
                    LedBulb_parameter.读写设备_地址_具体地址 = _Parameter.读写设备_地址_具体地址;//获取对象
                    #endregion
                    model.SaveChanges();//执行操作
                }
                //执行修改切换开关类字体参数操作
        ...
        public string Button_Parameter_modification(string ID, control_location _Location)//指示灯类ID修改参数--坐标参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                //执行修改切换开关类坐标参数操作
