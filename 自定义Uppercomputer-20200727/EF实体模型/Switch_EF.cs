...
namespace 自定义Uppercomputer_20200727.EF实体模型
{
    class Switch_EF
    {
        public static string Button_Parameter_inquire(string ID)//切换开关类参数ID查询
        {
            ...
            {
                Switch_Class Switch_Parameter = model.Switch_Class.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID
        ...
        public string Button_Parameter_Add(Switch_parameter parameter)//切换开关类主参数参数插入
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                Switch_parameter Switch_Parameter = model.Switch_parameter.Where(pi => pi.ID == parameter.ID).FirstOrDefault();//查询数据库是否有该ID
                if (Switch_Parameter == null)
                {
                    Switch_parameter parameter1 = new Switch_parameter();//实例化对象
                    parameter1 = parameter;//传入获取到的对象
                    model.Switch_parameter.Add(parameter1);//构造添加表的SQL语句
        ...
        public Switch_Class Button_Parameter_Query(string ID)//查询切换开关类全部参数
        {
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                Switch_Class button_Class = model.Switch_Class.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID
        ...
        public string Button_Parameter_delete(string ID)//切换开关类ID删除参数
        {
            ...
            {
                //执行删除切换开关类主参数操作
                Switch_parameter Switch_Parameter = model.Switch_parameter.Where(pi => pi.ID == ID).FirstOrDefault();//查询数据库是否有该ID   
                if (Switch_Parameter != null)
                {
                    model.Switch_parameter.Remove(Switch_Parameter);////构造添加表的SQL语句
                    model.SaveChanges();//执行操作
                }
                //执行删除切换开关类字体参数操作
        ...
        public string Button_Parameter_modification(string ID, Switch_parameter _Parameter, Tag_common_parameters tag_Common_,
            General_parameters_of_picture _Of_Picture, control_location _Location,Button_colour button_)//切换开关类ID修改参数
        {
            ...
            using (UppercomputerEntities2 model = new UppercomputerEntities2())
            {
                ...
                {
                    #region 要修改的属性
                    Switch_parameter.位切换开关 = _Parameter.位切换开关;//获取对象
                    Switch_parameter.位指示灯 = _Parameter.位指示灯;//获取对象
                    Switch_parameter.写设备_地址_具体地址_复选 = _Parameter.写设备_地址_具体地址_复选;//获取对象
                    Switch_parameter.写设备_地址_复选 = _Parameter.写设备_地址_复选;//获取对象
                    Switch_parameter.写设备_复选 = _Parameter.写设备_复选;//获取对象
                    Switch_parameter.操作安全时间 = _Parameter.操作安全时间;//获取对象
                    Switch_parameter.操作模式 = _Parameter.操作模式;//获取对象
                    Switch_parameter.读写不同地址_ON_OFF = _Parameter.读写不同地址_ON_OFF;//获取对象
                    Switch_parameter.读写设备 = _Parameter.读写设备;//获取对象
                    Switch_parameter.读写设备_地址 = _Parameter.读写设备_地址;//获取对象
                    Switch_parameter.读写设备_地址_具体地址 = _Parameter.读写设备_地址_具体地址;//获取对象
                    #endregion
                    model.SaveChanges();//执行操作
                }
                //执行修改切换开关类字体参数操作
